...
def is_identifier(string):
    """
	"""
    Identifier = pyp.Word(pyp.alphas + "_", bodyChars=pyp.alphanums + "_-.")
    try:
...
def get_args(args):
	...
	"""
    return [
        handle_expression(i) if isinstance(i, dict) else handle_value(i) for i in args
    ]
...
def handle_expression(dictn):
    ...
    # UNDO(Pebaz):
    the_args = [
        getkey(i) if isinstance(i, dict) else get_arg_value(i) for i in getvalue(dictn)
    ]
    ...
    # These Exceptions are not errors so pass them on to be caught by craft_call
        ...
        CraftFunctionReturnException,
        CraftLoopBreakException,
        CraftLoopContinueException,
    ) as e:
        raise e
