...
def craft_parse(text):
    ...
    BlockComment = pyp.Combine(
        pyp.Literal(":>") + pyp.SkipTo(pyp.Literal("<:")) + pyp.Literal("<:")
    ).suppress()
    ...
    Function << pyp.Dict(
        pyp.Group(
            ...
            + pyp.Literal(":")
            + pyp.Group(
                LBRACKET + pyp.ZeroOrMore(Comment | Function | List | Value) + RBRACKET
            )
        )
    )

    List << pyp.Group(LBRACKET + pyp.ZeroOrMore(Comment | Value | List) + RBRACKET)
